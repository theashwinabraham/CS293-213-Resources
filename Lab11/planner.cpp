...
bool Planner::doUserJob() {
  ...
  bool contFlag;
  int maxStopOvers, maxTransitTime, numDestStns;
  listOfObjects<string> * destStnNameList, *currDestStn;  
  ...
  do {
    if (!readFrmFile) {
      ...
      cout << "  170. Optimize monthly plan" << endl;
      cout << endl << "Input: ";
    }
	...
	continue;
      case USER_MONTHLY_PLAN:
	logFile << "Executing \"Find optimal monthly plan\"" << endl;
	...
	if (!readFrmFile) {
	  cout << "Enter count of destination stations in plan: ";
	}
	numDestStns = getInt(readFrmFile, &inpFile);

	if (numDestStns <= 0) {
	  logFile << "Incorrect number of destination stations" << endl;
	  cout << "Incorrect number of destination stations" << endl;
	  continue;
	}

	destStnNameList = nullptr;
	currDestStn = nullptr;
	contFlag = true;
	
	for (int i=0; i <  numDestStns; i++) {
	  if (!readFrmFile) {
	    cout << "Enter prefix of any word in destination[" << i+1 << "] station name: ";
	  }
	  ...
	  currDestStn = new listOfObjects<string> (destStnName);
	  if (currDestStn == nullptr) {
	    logFile << "Memory allocation error." << endl;
	    cout << "Memory allocation error." << endl;
	    contFlag = false;
	  }
	  currDestStn->next = destStnNameList;
	  if (destStnNameList != nullptr) {
	    destStnNameList->prev = currDestStn;
	  }
	  destStnNameList = currDestStn;
	}

	printMonthlyPlan(srcStnName, destStnNameList);
